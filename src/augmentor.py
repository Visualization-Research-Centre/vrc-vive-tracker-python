        ...
        if num_devices == slider_value:
            ...
            for i in range(slider_value - num_devices):
                device = copy.deepcopy(
                    decoded_data[i % num_devices]
                )  # Use modulo to cycle through existing devices
                device["name"] = self.fake_tracker_names[
                    i
                ...
                angle = (2 * math.pi / (slider_value - num_devices)) * (
                    i + 1
                )  # Shift by 1 to avoid zero angle
...
if __name__ == "__main__":
    ...
    assert (
        len(augmented_data) == slider_value
    ), "Number of devices is not equal to slider_value"
    print("\nTest 2 passed!")
