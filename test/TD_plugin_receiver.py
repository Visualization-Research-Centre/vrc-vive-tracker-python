...
def onReceive(dat, rowIndex, message, byteData, peer):
    ...
    for device in tracker_decoder.trackers:        
        # get position vector
        pos = device['position']
        ...
        if vr_tracker_devices_count > 0:
            ...
            for _ in range(vr_tracker_devices_count):
                ...
                if self.ignore_tracking_reference:
                    if (vr_tracker_device['name'].lower() in self.ignored_tracker_names 
                        or vr_tracker_device['name'].upper() in self.ignored_tracker_names 
